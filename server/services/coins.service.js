...
async function fetchCoins(req) {
    try {
        const response = await axios.get(`${process.env.COINGECKO_API_BASE_URL}/coins/markets`, {
            params: {
                vs_currency: 'usd',
...
async function fetchCoinDetails(req) {
    const response = await axios.get(`${process.env.COINGECKO_API_BASE_URL}/coins/${req.params.id}`, {
        params: {
            localization: false,
            tickers: false,
            market_data: true,
